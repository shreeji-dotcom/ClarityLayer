...
DEADLINE_PATTERN = re.compile(
    r"\bby\s+(eod|eow|cob|tomorrow|today|monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b"
...
def _build_email_summary(
    ...
    summary_parts = [f"Context: {opening}"]
...
def _apply_audience_mode(text: str, audience_mode: str) -> str:
    if audience_mode == "For Leadership":
        return (
            ...
            "- Next update: include progress against agreed KPI"
        )
...
def generate_workplan(project_description: str, audience_mode: str = "For Team") -> Dict[str, object]:
    ...
    deliverables = _recommended_deliverables(matched_categories)
    questions = _key_questions(matched_categories)
    missing_flags = _project_missing_flags(project_description, matched_categories)
    phase_plan = _build_phase_plan(matched_categories)

    explain_logic = []
    for category, hits in category_hits.items():
        explain_logic.append(f"Category '{category}' matched with {hits} keyword hit(s).")
    for flag in missing_flags:
        explain_logic.append(f"Quality gate trigger: {flag}.")

    objective_tags: List[str] = []
    if any("owner" in f.lower() for f in missing_flags):
        objective_tags.append("Missing owner")
    if any("due date" in f.lower() for f in missing_flags):
        objective_tags.append("Missing due date")
    if any("success metric" in f.lower() for f in missing_flags):
        objective_tags.append("Missing success metric")
    if any("dependency" in dep["dependency"].lower() for dep in _dependency_map(matched_categories)):
        objective_tags.append("Dependency mapped")
    if any(k in normalized for k in ["kpi", "metric"]):
        objective_tags.append("KPI term found")
    ...
    return {
        ...
        "phase_1": [
            "Confirm business objective, measurable outcomes, and scope boundaries.",
            "Map stakeholders, decision owners, and governance cadence.",
            "Define assumptions, constraints, and success criteria.",
        ],
        "phase_2": [
            "Collect source materials, baseline data, and process evidence.",
            "Validate definitions, data quality, and dependency assumptions.",
            "Run stakeholder checkpoints to confirm facts and gaps.",
        ],
        "phase_3": [
            "Analyze root causes and quantify impact opportunities.",
            "Build options with trade-offs, risk, and implementation effort.",
            "Recommend target-state plan with timeline and ownership.",
        ],
        "phase_plan": phase_plan,
        "workstreams": _workstreams(matched_categories),
        "success_metrics": _success_metrics(matched_categories),
        "dependency_map": _dependency_map(matched_categories),
        "key_questions": questions,
        "missing_info_flags": missing_flags,
        "explain_logic": explain_logic,
        "objective_tags": objective_tags,
        "audience_note": _apply_audience_mode("Workplan generated.", audience_mode),
        "risks_watchouts": [
            "Ambiguous ownership can stall key decisions.",
            "Data inconsistency can undermine recommendation quality.",
            "Scope growth without change control can delay delivery.",
            "Insufficient adoption planning can reduce realization of value.",
        ],
    }
